 ...
 */
router.post('/signup', signupLimiter, ...(signupValidation as RequestHandler[]), validate, async (req: Request, res: Response) => {
  try {
    const { 
      full_name, 
      email, 
    ...
    res.status(201).json({
      ...
      data: {
        ...
        email: data.email,
        signup_date: (data as any).signup_date
      }
    })
 ...
 */
router.get('/signups', authenticateToken, async (req: Request, res: Response) => {
  try {
    if (!supabaseAdmin) {
 ...
 */
router.get('/signups/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    if (!supabaseAdmin) {
      return res.status(503).json({ 
        error: 'Admin access not configured'
 ...
 */
router.patch('/signups/:id/status', authenticateToken, async (req: Request, res: Response) => {
  try {
    if (!supabaseAdmin) {
      return res.status(503).json({ 
        error: 'Admin access not configured'
 ...
 */
router.put('/signups/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    if (!supabaseAdmin) {
      return res.status(503).json({ 
        error: 'Admin access not configured'
 ...
 */
router.post('/signups', authenticateToken, async (req: Request, res: Response) => {
  try {
    if (!supabaseAdmin) {
      return res.status(503).json({ 
        error: 'Admin access not configured'
 ...
 */
router.delete('/signups/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    if (!supabaseAdmin) {
      return res.status(503).json({ 
        error: 'Admin access not configured'
